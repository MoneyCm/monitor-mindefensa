...
class PDFGenerator:
    ...
    def generar(self, resultados, output_pdf="reporte_observatorio.pdf"):
        ...
        # Tabla 1: Comparativo Acumulado YTD (Top 5)
        ...
        ytd_table_data = [[
            Paragraph("Delito", style_th), 
            Paragraph(f"{anio_ant} acum.", style_th), 
            Paragraph(f"{anio_act} acum.", style_th), 
            Paragraph("Diferencia", style_th), 
            Paragraph("Variación %", style_th)
        ]]

        ytd_rows = sorted(resumen_tabla, key=lambda x: x['actual'], reverse=True)[:5]
        for row in ytd_rows:
            ...
            ytd_table_data.append([
                Paragraph(row['delito'], style_td_left),
                Paragraph(f"{row['anterior']:,}", style_td),
                Paragraph(f"<b>{row['actual']:,}</b>", style_td_bold),
                Paragraph(f"<b>{row['diffAbs']:+,}</b>", ParagraphStyle('d', alignment=TA_CENTER, textColor=color_var, fontSize=8)),
                Paragraph(f"<b>{row['varPct']}</b>", ParagraphStyle('v', alignment=TA_CENTER, textColor=color_var, fontSize=8))
            ])
        ...
        # Tabla 2: Comparativo Puntual del Periodo (Top 5)
        ...
        puntual_table_data = [[
            Paragraph("Delito", style_th), 
            Paragraph(f"Mismo Per. {anio_ant}", style_th), 
            Paragraph("Per. Actual", style_th), 
            Paragraph("Per. Anterior", style_th), 
            Paragraph("Var. YoY", style_th), 
            Paragraph("Var. PoP", style_th)
        ]]

        puntual_rows = sorted(resumen_puntual, key=lambda x: x['per_act'], reverse=True)[:5]
        for row in puntual_rows:
            ...
            # Formatear el nombre del delito
            delito_label = row['delito']
            
            puntual_table_data.append([
                Paragraph(delito_label, style_td_left),
                Paragraph(f"{row['mismo_per']:,}", style_td),
                Paragraph(f"<b>{row['per_act']:,}</b>", style_td_bold),
                Paragraph(f"{row['per_ant']:,}", style_td),
                Paragraph(f"<b>{row['diff_yoy']:+,}</b>", ParagraphStyle('dy', alignment=TA_CENTER, textColor=color_yoy, fontSize=8)),
                Paragraph(f"<b>{row['diff_pop']:+,}</b>", ParagraphStyle('dp', alignment=TA_CENTER, textColor=color_pop, fontSize=8))
            ])
